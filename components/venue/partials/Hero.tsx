...
const HeroSection: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {/* Main content */}
      <div className="relative z-30 flex-1 flex items-end px-8 md:px-16 pb-12 md:pb-16">
        <div className="w-full max-w-7xl mx-auto flex flex-col items-start gap-8">

          {/* Eyebrow tag */}
          <div className="overflow-hidden">
            <div className="hero-line border border-white/20 rounded-full px-5 py-2.5 inline-flex">
          ...
          {/* Heading */}
          <h1 className="font-serif text-8xl leading-[1.05] tracking-[-0.01em] text-white">
          ...
          {/* Sub-headline */}
          <p className="hero-fade-body max-w-[50ch] text-white/90 text-xl font-normal leading-[1.8] font-sans">
            {sub_headline}
          </p>

          {/* Event type chips */}
          <div className="flex flex-wrap items-center gap-2">
            <span className="hero-chip font-sans text-[9px] tracking-[0.42em] uppercase text-white/40 mr-1">
              Hosting
            </span>
            {HOSTED_EVENTS.map((t) => (
              <span
                key={t}
                className="hero-chip font-serif text-[12.5px] text-white/70 px-3.5 py-1.5 border border-white/20 rounded-full hover:border-[#ED80A8] hover:text-[#ED80A8] transition-colors cursor-default"
              >
                {t}
              </span>
            ))}
          ...
          <div className="flex flex-wrap items-end gap-10 pt-2">
            {/* Buttons */}
            <div className="hero-fade-body flex flex-wrap items-center gap-4">
              <a
                href={buttons[0].link}
                className="bg-[#a85b74] text-white text-[12px] font-bold tracking-[0.2em] uppercase px-10 py-4 rounded-full hover:bg-white hover:text-[#1a1014] transition-all"
              >
                Book a Tour
              </a>
              <a
                href="#"
                className="text-white text-[12px] font-semibold tracking-[0.2em] uppercase px-10 py-4 rounded-full border border-white/30 hover:bg-white/10 transition-all"
              >
                View Packages
              </a>
            </div>
            ...
            <div className="grid grid-cols-3 gap-8">
              {[
                ...
                <div key={s.label}>
                  <div className="overflow-hidden">
                    <p className="hero-stat-num font-serif text-[clamp(22px,2.5vw,36px)] leading-none text-white tabular-nums">
                      {s.num}
                    </p>
                  </div>
                  <div className="mt-2 h-px w-full bg-[#ED80A8]/30" />
                  <p className="mt-2 font-sans text-[9px] tracking-[0.36em] uppercase text-white/40">
                    {s.label}
                  </p>
                </div>
              ))}
