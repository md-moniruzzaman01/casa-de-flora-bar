...
// Aligned with backend table slots (10:00, 12:00, 14:00, 16:00, 18:00, 20:00) at 90-min duration.
const TIME_SLOTS = [
  "10:00 AM – 11:30 AM",
  "12:00 PM – 1:30 PM",
  "2:00 PM – 3:30 PM",
...
// Display label → backend "HH:mm" (matches backend src/config/slots.ts).
const TIME_SLOT_TO_BACKEND: Record<string, string> = {
                  ...
                  <label key={b} className="flex items-center gap-2.5 cursor-pointer group/check">
                    <div
                      ...
                      className={`w-4 h-4 rounded flex items-center justify-center border transition-colors flex-shrink-0 ${
                        local.bookingFor.includes(b) ? "bg-pink-500 border-pink-500" : "border-gray-300 group-hover/check:border-pink-300"
                      }`}
                    >
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between flex-shrink-0">
          <div>
            <h2 className="text-base font-semibold text-gray-900">Edit Reservation</h2>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
          ...
          <section>
            ...
            <div className="space-y-3">
              <div>
                <label className="text-xs text-gray-500 block mb-1.5">Booking Type</label>
                <select
                  value={form.bookingFor}
                  onChange={e => set("bookingFor", e.target.value)}
                  className={`${inp} cursor-pointer`}
                >
                  {!BOOKING_TYPES.includes(form.bookingFor) && (
                    <option value={form.bookingFor}>{form.bookingFor}</option>
                  )}
          ...
          <section>
            ...
            <textarea
              ...
              rows={3}
              placeholder="Special requests, allergies, celebration notes…"
...
function AddDrawer({ onClose, onCreated }: {
  ...
  async function handleSave() {
    ...
    const timeSlot = TIME_SLOT_TO_BACKEND[form.time];
    if (!timeSlot) {
      setError("Please pick a valid time slot.");
      return;
    }
  ...
  const inp =
    "w-full border border-gray-200 px-3.5 py-2.5 rounded-xl text-sm text-gray-800 bg-white focus:outline-none focus:ring-1 focus:ring-pink-300 focus:border-pink-300 transition-all";
      ...
      >
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between flex-shrink-0">
          <div>
            ...
            <p className="text-[11px] text-gray-400 mt-0.5">Create a table booking</p>
          </div>
          <button
            onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-100 text-gray-400 transition-colors"
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
          ...
          <section>
            <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-gray-400 mb-3">Booking Details</p>
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-gray-500 block mb-1.5">Date</label>
          ...
          <section>
            ...
            <textarea
              ...
              onChange={e => set("notes", e.target.value)}
              rows={3}
              placeholder="Allergies, celebration notes…"
              className={`${inp} resize-none`}
            />
          </section>
...
export default function TableReservations() {
  ...
  async function handleSave(updated: Reservation) {
    ...
    try {
      await api.patch(`/api/table-bookings/${updated.id}`, {
        ...
        timeSlot: TIME_SLOT_TO_BACKEND[updated.time] ?? updated.time,
  ...
  const processed = useMemo(() => {
    ...
    if (filters.bookingFor.length) rows = rows.filter(r => filters.bookingFor.includes(r.bookingFor));
  ...
  const stats = useMemo(() => {
    ...
    const today    = data.filter(r => r.dateTs >= startToday && r.dateTs < endToday).length;
    const thisWeek = data.filter(r => r.dateTs >= startToday && r.dateTs <  endWeek ).length;
    const pending  = data.filter(r => r.status === "Pending").length;
    const totalGuests = data.reduce((sum, r) => sum + r.guests, 0);
    const avgGuests   = data.length > 0 ? Math.round(totalGuests / data.length) : 0;

    return { today, thisWeek, pending, avgGuests };
  }, [data]);

  return (
    <>
      {/* ── Edit drawer (portal-like, renders above everything) ── */}
      {editing && (
        <EditDrawer
          reservation={editing}
      ...
      <div className="flex-1 flex flex-col min-h-screen bg-white">
        <main className="flex-1 px-8 py-7 flex flex-col gap-7">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard label="Today"          value={stats.today}      sub="reservations today"          />
            <StatCard label="This Week"      value={stats.thisWeek}   sub="next 7 days"                 />
            <StatCard label="Pending"        value={stats.pending}    sub="needs approval"              />
            <StatCard label="Avg group size" value={stats.avgGuests}  sub="per booking"                 />
          </div>
              ...
              <div className="flex flex-wrap gap-2 items-center">
                ...
                {filters.bookingFor.map(b => (
                  <span key={b} className="inline-flex items-center gap-1.5 px-3 py-1 bg-pink-50 border border-pink-100 rounded-full text-xs text-pink-700">
                    {b}
                    <button onClick={() => removeBookingFilter(b)} className="text-pink-400 hover:text-pink-600"><X size={11} /></button>
                  </span>
                ))}
                {filters.guestCount !== "any" && (
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-pink-50 border border-pink-100 rounded-full text-xs text-pink-700">
                    ...
                    <button onClick={() => applyFilters({ ...filters, guestCount: "any" })} className="text-pink-400 hover:text-pink-600"><X size={11} /></button>
                  </span>
                )}
                {filters.dateRange !== "any" && (
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-pink-50 border border-pink-100 rounded-full text-xs text-pink-700">
                    {DATE_RANGE_OPTIONS.find(o => o.value === filters.dateRange)?.label}
                    <button onClick={() => applyFilters({ ...filters, dateRange: "any" })} className="text-pink-400 hover:text-pink-600"><X size={11} /></button>
                  </span>
                )}
                <button
                ...
                <tbody className="divide-y divide-gray-50">
                  {paginated.map((r, idx) => (
                    ...
                    >
                      ...
                      {/* Action cell — stops row-click propagation */}
